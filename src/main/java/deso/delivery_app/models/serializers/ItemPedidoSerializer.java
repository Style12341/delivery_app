...
public class ItemPedidoSerializer implements ISerializable<ItemPedido>{
    ...
    public String getUpdateString(ItemPedido i) {
        return String.format(Locale.US,"UPDATE Item_pedido SET cantidad=%d, precio_total=%f, item_menu_id=%d, pedido_id=%d WHERE id=%d",
                i.getCantidad(), i.getPrecioTotal(), i.getItemMenu().getId(), i.getPedido().getId(), i.getId());
    }
